...
public class MyRenderClass implements Runnable{
    ...
    protected void createFont(String path, float size, boolean bold){
        java.awt.Font baseFont;
        baseFont = Font.createFont(Font.TRUETYPE_FONT, new InputStream(path) );
        Font awtFont;
        if(bold)
            awtFont = baseFont.deriveFont(Font.BOLD, size);
        else
            awtFont = baseFont.deriveFont(Font.PLAIN, size);
